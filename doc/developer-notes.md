...
- [Development guidelines](#development-guidelines)
    - [General Bitx Core](#general-bitx-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bitx Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BXs (tBX)" on the test network, if you
...
([`valgrind.supp`](https://github.com/bitx-project/bitx/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bitx
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bitx --log_level=test_suite
$ valgrind -v --leak-check=full src/bitxd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bitx.coverage/index.html`.
```
...
- BitcoinMiner : Generates BXs (if wallet is enabled).
...
However, in open source software such as Bitx Core, where everyone uses
...
be sent upstream but bugfixes may also be prudent to PR against Bitx Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Bitx-Project/Bitx.git

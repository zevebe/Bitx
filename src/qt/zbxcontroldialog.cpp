...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbxTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbxTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBX.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBX seed used to mint this zBX is not the same as currently hold in the wallet";
            else

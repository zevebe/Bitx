...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZBXModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zbxTracker->HasMintTx(hash))) {
            //zBX stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZBX;
...
bool IsZBXType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZBX:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZBX || type == TransactionRecord::MNReward) {

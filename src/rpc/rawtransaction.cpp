...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of bitx addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) bitx address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"address\" : \"address\",  (string) the bitx address\n"
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the bitx address, the value is the btc amount\n"
            "      ,...\n"
            "    }\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) bitx address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nCreates raw zBX coinstakes (without MN output).\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    // create the zerocoinmint output (one spent denom + three 1-zBX denom)
    ...
    if (!pwalletMain->CreateZBXOutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zbx output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZBXOutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zbx output");
    }

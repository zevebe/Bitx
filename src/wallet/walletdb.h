...
public:
    ...
    bool WriteZBXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBXSeed_deprecated(uint256& seed);
    bool EraseZBXSeed();
    bool EraseZBXSeed_deprecated();

    bool WriteZBXCount(const uint32_t& nCount);
    bool ReadZBXCount(uint32_t& nCount);

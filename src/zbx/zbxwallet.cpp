...
CzBXWallet::CzBXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBXSeed_deprecated()) {
                    LogPrintf("%s: Updated zBX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBXCount(nCountLastUsed);
    else if (!walletdb.ReadZBXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBXWallet::Lock()
{
    seedMaster = 0;
}

void CzBXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBXWallet::LoadMintPoolFromDB()
{
...
void CzBXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbxTracker which also adds to database
    pwalletMain->zbxTracker->Add(dMint, true);
...
void CzBXWallet::SeedToZBX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBXWallet::GenerateDeterministicZBX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

...
CzBXTracker::CzBXTracker(std::string strWalletFile)
{
...
CzBXTracker::~CzBXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBXTracker::Init()
{
...
bool CzBXTracker::Archive(CMintMeta& meta)
{
...
bool CzBXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBXTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBXTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBXTracker::GetSerialHashes()
{
...
CAmount CzBXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBXWallet* zBXWallet)
{
    bool iszBXWalletInitialized = (NULL != zBXWallet);
    CMintMeta meta;
    ...
    if (! iszBXWalletInitialized)
        zBXWallet = new CzBXWallet(strWalletFile);
    meta.isSeedCorrect = zBXWallet->CheckSeed(dMint);
    if (! iszBXWalletInitialized)
        delete zBXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBXWallet* zBXWallet = new CzBXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBXWallet);
        }
        delete zBXWallet;
        LogPrint("zero", "%s: added %d dzbx from DB\n", __func__, listDeterministicDB.size());
    }

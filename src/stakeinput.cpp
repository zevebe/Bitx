...
// The zBX block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBX is a hash of the serial
...
{
    //Create an output returning the zBX that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBXOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBX output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBX", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBXOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBX output", __func__);
...
{
    CzBXTracker* zbxTracker = pwallet->zbxTracker.get();
    CMintMeta meta;
    if (!zbxTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbxTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BX stake is the outpoint
